...
public class Asserts {
    ...
    public static void isNotMax(String str, Integer maxLength, String message) {
        if (str == null || str.trim().length() > maxLength) {
            throw new AssertsException(LinkUtils.link(message, "长度不能超过", maxLength, "个字符"));
        }
    }
    ...
    public static void mayNotMax(String str, Integer maxLength, ResultCodeEnum codeEnum) {
        if (Objects.isNull(str)) {
            return;
        }
    ...
    public static void mayScope(BigDecimal val, Integer start, Integer end, String message) {
        ...
        if (val.doubleValue() < start.doubleValue() || val.doubleValue() > end.doubleValue()) {
            throw new AssertsException(LinkUtils.link(message, "必须大于等于", start, "并且小于等于", end));
        }
    }
    ...
    public static <T, R> void isValidBatch(List<T> paramList, Function<R, T> resultFunc, List<R> resultList, String message) {
        ...
        if (!notValidParamList.isEmpty()) {
            throw new AssertsException(LinkUtils.link(message, ",非法参数为", notValidParamList));
        }
    }

    public static void isScope(BigDecimal value, BigDecimal min, BigDecimal max, String message) {
        if (value == null
                || value.compareTo(min) < 0
                || value.compareTo(max) > 0) {
            throw new AssertsException(LinkUtils.link(message, "超出数值范围,最小值:", min, ", 最大值:", max));
        }
    }
    ...
    public static void isEqualScope(BigDecimal value, BigDecimal min, BigDecimal max, String message) {
    ...
    public static void isMayEqualScope(BigDecimal value, BigDecimal min, BigDecimal max, String message) {
        if (value == null) {
            return;
        }
